...
Seal/verify inventory through **schema 49** is complete. Soft mirror-only schema
...
- Fanuc mill lint Phase A scaffolded from Haas: added the portable first rule slice, Fanuc-specific dialect exclusions, and a roadmap for later dedupe.
- Haas mill lint batch 90: added semantic M98/M97 same-block stop/end conflicts, S-word while through-spindle coolant is active, and M98 non-XY plane hygiene.
- Haas mill lint batch 89: added G65 macro-call same-block checks against cutter/tool-length/rotation/scaling apply, G80/G40/G49/G69/G50 cancels, and work-offset selects.
...
- Haas mill lint batches: added H/D/IJK orphan-offset checks while through-spindle coolant (`M88`) or spindle orientation (`M19`) is active, and started `M30` while-state mirroring of `M02` for coolant/cutter/canned/tool-length/rotation; finished rotary/Q/R/P/IJK orphan checks while spindle orientation (`M19`) is latched or through-spindle coolant (`M88`) is active; repeated `M19`, `M89`, `M9`, orphan `F`/`L`/`A` while spindle orientation (`M19`) is latched or through-spindle coolant (`M88`) is active, and orphan `B` while through-spindle coolant is active; `G0` rapid, `M00`/`M01` stops, coolant on (`M7`/`M8`), `M9`, redundant `M88`, and bare `M89` hygiene for through-spindle coolant (`M88`) (turn off with `M89`), plus `G1`/`G2`/`G3` feed motion, spindle-speed `S`, and canned cycles while spindle orientation (`M19`) is still latched (clear with `M3`/`M4`/`M5`); `M19` spindle-orient while through-spindle coolant (`M88`) is still active (turn off with `M89`), plus `M02`/`M30` program-end, `G4` dwell, and `T` tool-select while through-spindle coolant (`M88`) is still active (turn off with `M89`), and `M02`/`M30` program-end, `G0` rapid, `G4` dwell, and `T` tool-select while spindle orientation (`M19`) is still latched (clear with `M3`/`M4`/`M5`); `G69`/`G50` cancels, `G41`/`G42` cutter comp, `G43` tool length, `G68` rotation, `G51` scaling, `M88` through-spindle coolant, coolant on (`M7`/`M8`), and `M00`/`M01` stops while spindle orientation (`M19`) is still latched (clear with `M3`/`M4`/`M5`); `G92`/`G52`/plane (`G17`/`G18`/`G19`)/unit (`G20`/`G21`)/feed-mode (`G93`/`G94`/`G95`)/path-mode (`G61`/`G64`)/distance-mode (`G90`/`G91`)/`G40`/`G49`/`G80` while spindle orientation (`M19`) is still latched (clear with `M3`/`M4`/`M5`); `M6`/`G28`/`G30`/`G53`/`M98`/`M97`/`G65`/`M99`/`G10`/work-offset (`G54`-`G59`/`G154`) while spindle orientation (`M19`) is still latched (clear with `M3`/`M4`/`M5`); `G40`/`G49`/`G69`/`G50`/`G80` cancels, `G41`/`G42` cutter comp, `G43` tool length, `G68` rotation, `G51` scaling, and distance-mode (`G90`/`G91`) select while through-spindle coolant (`M88`) is still active (turn off with `M89`); `M97`/`M99`/`G10`/`G92`/`G52`/work-offset (`G54`-`G59`/`G154`)/plane (`G17`/`G18`/`G19`)/unit (`G20`/`G21`)/feed-mode (`G93`/`G94`/`G95`)/path-mode (`G61`/`G64`) while through-spindle coolant (`M88`) is still active (turn off with `M89`); `M19` spindle-orient while the spindle is off, plane (`G17`/`G18`/`G19`) and distance-mode (`G90`/`G91`) selects while the spindle is still on, and `M5`/`M6`/`G28`/`G30`/`G53`/`M98`/`G65` while through-spindle coolant (`M88`) is still active (turn off with `M89`); `G41`/`G42` cutter comp, `G93`/`G95`/`G94` feed modes, `G68` rotation, and `G51` scaling activated while the spindle is off, plus work-offset (`G54`-`G59`/`G154`), path-mode (`G61`/`G64`), feed-mode (`G93`/`G94`/`G95`), and unit (`G20`/`G21`) selects while the spindle is still on; finished `M89` TSC-off while-state — rotation/scaling/`G91`/flood-mist coolant/tool-length still active and while the spindle is off — plus `M98`/`M97`/`G65`/`M99` subprogram/macro calls and return while the spindle is still on; `M88` through-spindle coolant while cutter comp/canned/rotation/scaling/`G91`/flood-mist coolant/tool-length still active and while the spindle is off, plus `M89` TSC-off while cutter comp or a canned cycle is still active; `M19` spindle-orient while cutter comp/canned/rotation/scaling/`G91`/coolant/tool-length still active, and `G50`/`G69`/`G80` cancel ops while the spindle is still on; `G93` inverse-time feed select while coolant/`G91` still active, and `G10`/`G92`/`G52`/`G28`/`G30`/`G53`/`G68`/`G51` positioning/coordinate ops while the spindle is still on; `G10` data setting while rotation/scaling/`G91`/coolant/tool-length still active, and `G93` inverse-time feed select while cutter comp/canned/rotation/scaling/tool-length still active; same-block mutually exclusive modal pairs (`G90`+`G91`, `G17`/`G18`/`G19` planes, `G20`+`G21`, `G94`+`G95`, `G61`+`G64`), mixed `G93`/`G95` feed modes, and `G10` data setting while cutter comp or a canned cycle is still active; `M02` program-end while-state (coolant/cutter/canned/tool-length/rotation/scaling/`G91`) mirroring `M00`/`M01`, `P`/`R` orphan while a canned cycle is active, and `M00`/`M01` followed by a `-Z` move before spindle restart; spindle-speed `S` while `G43`, `G0` while `G91`, distance-mode/canned cycle while `G43`, canned while coolant on, missing feed mode (`G93`/`G94`/`G95`) and plane mode (`G17`/`G18`/`G19`) before motion, `G93`+`G94`/`G95` same-block, and mixed `G93`/`G94`; careful orphan F-word while-state (cutter/canned/rotation/scaling/incremental/coolant/tool-length, outside G1/G2/G3 or canned feed context) plus A/H/D offset words while `G43` tool length is active; finished B/C rotary orphan while-state modes (rotation/scaling/incremental/coolant/tool-length); cancel-family matrix, G40/G80/G0/spindle/coolant while G43, G0 while coolant, and finished A-axis orphan modes; G93/M19/M88 end hygiene and context-aware A/B/C rotary orphan checks; orphan L-word while-state (table-driven modes), Q while G43, missing unit mode before motion, and G10-at-end hygiene; `H0`/`D0` offsets, coolant before spindle, `F0`, arcs without `R`/`IJK`, cutter comp left on at end; canned cycle Z/R/`G80`/M6 safety and mixed `G20`/`G21`; end-of-program cleanup for spindle/`M5`, coolant/`M9`, `G43`/`G49`, and `G91`/`G90`; work-offset/G53/`M6`+spindle/plane hygiene; feed-with-spindle-off, rapid `-Z` without `G43`, cutter-comp+`G0`, and `M98`/`M97` without `P`; `G65`/`G4`, H≠T pairing, mid-program offset switches, and multi-axis `G28`; arc R+IJK conflict, `G94`/`G95` mix, missing `O` header, multi-axis `G30`, and spindle direction conflicts; canned `F`/`Q`, `G68` cancel, G41↔G42 without `G40`, and `G43` before `T`; distance mode, unit switch after motion, tapping without spindle, `G51` cancel, and `G43`+`G49` same block; plane/feed switches after motion, `G61`/`G64` mix, and spindle/coolant on+off same block; distance switch after motion, `G40`/`G80` same-block cancel conflicts, and `M6` with cutter/`G43` still active; path switch after motion, G41+G42/`M7`+`M8` same-block, `M6` with coolant on, and `G53`+work-offset conflict; `G68`/`G69`, `G50`/`G51`, `G28`/`G30` same-block, spindle reverse without `M5`, and coolant while spindle off; `M98`/`M97`, `G65`/`M98`, `M00`/`M01` same-block, and `G28`/`G30` while absolute; `G65`/`M97`, `M99`/`M30` same-block, `M6` with rotation/scaling, and `G92` caution; `M98`/`M99` same-block, bare `G28`/`G30`, `G28`+`G53`, and zero `G4` dwell; `G30`+`G53`, bare/multi-axis `G53`, `M97`/`M99` same-block, and `G4`+motion; `G65`/`M99`, `M6` in `G91`, `G4`+`M6`, and negative `F`/`S`; `G28`/`G53` while cutter or canned active, and `G52` caution; `G30` while cutter/canned, `G28`/`G53` while rotation, and `G28` while scaling; `G53`/`G30` while scaling, `G30` while rotation, and `G28`/`G53`+`G92` same-block; `G30`+`G92`, `G28`/`G53`/`G30`+`G52`, and `G92`+`G52` same-block; `M6`+`G28`/`G30`/`G53` and `G4`+`G28`/`G30` same-block; `G4`+`G53`, `M6`+`G65`, and `M98`/`M97`/`G65`+`G28` plus `M98`+`G53` same-block; collapsed pairwise M+M conflicts into `haas.multiple-m-codes-same-block` (Haas one-M-per-block); `M97`/`G65`+`G53`, `M98`/`M97`/`G65`+`G30`, and `M00`/`M01`+`G28`/`G53` plus `M00`+`G30` same-block; `M01`+`G30`, `M99`+`G28`/`G30`/`G53`, and `G4`+`G65`/`M98`/`M97`/`M00`/`M01`/`M99` same-block; `M30`/`M02`+`G28`/`G30`/`G53` and `G4`/`M6`+`G92`/`G52` same-block; work-offset+`G28`/`G30` and `G43`/`G49`/`G40`/`G80`+`G92`/`G52` same-block; work-offset/`G41`/`G42`/`G68`/`G69`/`G51`+`G92`/`G52` same-block; `G50`+`G92`/`G52` and `G43`/`G49`/`G40`/`G80`+`G28`/`G30` same-block; `G43`/`G49`/`G40`/`G80`+`G53` and `G41`/`G42`/`G68`/`G69`+`G28`/`G30` same-block; collapsed home/shift pairwise matrices into `haas.machine-position-conflict-same-block` and `haas.coord-shift-conflict-same-block`; `G28`/`G30`/`G53` while `G43`, non-tapping canned/`G1` `-Z` without `G43`, cutter without `G43`, and `M98`/`M97` while cutter/canned; `M98`/`M97`/`G65` while tool-length/rotation/scaling and `G65` while cutter/canned; `G65` while scaling, call/macro while coolant/`G91`, and `M99` while cutter/canned/`G43`; `M99` while rotation/scaling/coolant/`G91`, `G28`/`G30`/`G53` while coolant, and `M5` while coolant/cutter/canned; `M5` while tool-length/rotation/scaling/`G91`, and `M00`/`M01` while coolant/cutter/canned; `M00`/`M01` while tool-length/rotation/scaling/`G91`, and `G4` while cutter/canned; `G4` while tool-length/rotation/scaling/coolant/`G91`, `G0` while canned, and work-offset while cutter/canned/rotation/scaling; work-offset while tool-length/coolant/`G91`, `G68`/`G51` while cutter/canned, and `G49`/`G80` cancel-order hygiene; `G68`/`G51` while tool-length/scaling/rotation, and `G40`/`G69`/`G50` cancel-order hygiene; `G49`/`G50`/`G69`/`G80` cancel-order with rotation/scaling, and `G68` while coolant; `G68`/`G51` while `G91`/coolant, and `G49`/`G69`/`G50`/`G80` cancel with coolant/`G91`; `G40` while coolant/`G91`, `G41`/`G42` while canned/rotation/scaling/`G91`, `G43` while cutter/canned, and canned while cutter/rotation; `G43` while rotation/scaling/coolant/`G91`, canned while scaling/`G91`, and plane select while cutter/canned/rotation/scaling; plane while coolant/`G91`, unit select while cutter/canned/rotation/scaling, and feed/path mode while cutter/canned; unit while coolant/`G91`, and feed/path mode while rotation/scaling/coolant/`G91`; distance mode while cutter/canned/rotation/scaling/coolant, plane/unit/feed/path while tool-length, and `G41`/`G42` while coolant; `G0` while rotation/scaling, `G80` while coolant, `M9` while cutter/canned/`G43`/rotation/scaling/`G91`, and spindle start while canned; spindle start while cutter/rotation/scaling/`G91`/coolant, coolant on while rotation/scaling/`G91`/cutter, and feed while canned; `G92`/`G52` while cutter/canned/rotation/scaling/tool-length; `G92`/`G52` while coolant/`G91`, coolant on while canned, feed after coolant off, and tool-select while cutter/canned/`G43`/rotation; tool-select while scaling/coolant/`G91`, spindle-speed while canned/cutter/rotation/scaling/`G91`, and end-of-program `G95`/`G61`; spindle-speed while coolant, `G52` still applied at end, and H/D offset words while canned/cutter/rotation/scaling/`G91`; H/D while coolant/`G91`/cutter finish, and Q-word while canned/cutter/rotation/scaling/`G91`/coolant; R-word while cutter/rotation/scaling/`G91`/coolant/`G43`, and P-word while cutter/rotation/scaling/`G43`; P-word while `G91`/coolant finish, I/J/K while cutter/rotation/scaling/`G91`/coolant/`G43`/canned, and end-of-program after `G92`; plus rules.meta coverage for first-G43 Z, early `M30`, and duplicate `N`.
- Soft seal/verify schema track frozen at **schema 49**; hard Known Gaps are demand-gated (PGP, full UAX#9, IDE-host write).
...
## CSV summary path + live desktop stamp + Schema v36

This wave drains five Known Gaps from the prior wave in one motion — all changes
are append-only with no new runtime dependencies.

### Move 1 — `csvSummaryPath` (Schema v36)

`CLI_SCHEMA_VERSION` bumps `35 → 36`.
`batchWalk.export` gains optional `csvSummaryPath`.

### Move 2 — CLI records `csvSummaryPath`

`--out-dir` predetermines and records the absolute path of `batch-summary.csv`
alongside the other summary sidecars (CSV was already always written).

### Move 3 — Desktop live stamp

`runDesktopBatchJobCheck` stamps relative logical `csvSummaryPath` /
`ndjsonSummaryPath` on live `batchWalk.export` for inventory parity with CLI.

### Move 4 — Inventory chip shows `csv`

When `csvSummaryPath` is present, the batch-export inventory chip includes
`csv`.
...
CLI envelope **schema 49** adds rule-policy / separable-rules foundation on top of the soft verify/inventory symmetry track
