...
import { openPolicyPanel, policyPresetSelect, policyRevertButton } from "./policy-helpers";
...
test("deep operator policy flow from edit to export confirmation", async ({ page }) => {
  ...
  await expect(page.locator("div", { hasText: /^preset=strict$/i })).toBeVisible();
  await expect(page.locator("div", { hasText: /^source=manual$/i })).toBeVisible();
  await expect(page.locator("div", { hasText: /^controller=fanuc$/i })).toBeVisible();
});
...
test("high-risk branch: manual override, drift, revert, and export", async ({ page }) => {
  ...
  await policyRevertButton(page).click();
  ...
  await expect(page.locator("div", { hasText: /^source=bootstrap$/i })).toBeVisible();
  await expect(page.locator("div", { hasText: /^controller=fanuc$/i })).toBeVisible();
});
...
test("controller-specific Haas branch: balanced default restore and export", async ({ page }) => {
  ...
  await expect(page.locator("div", { hasText: /^preset=balanced$/i })).toBeVisible();
  await expect(page.locator("div", { hasText: /^source=bootstrap$/i })).toBeVisible();
  await expect(page.locator("div", { hasText: /^controller=(haas-ngc|haas-legacy)$/i })).toBeVisible();
});
