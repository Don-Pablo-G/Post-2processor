...
function main() {
  const limit = parsePositiveInt(process.argv[2] ?? "10", 10);
  ...
  const payload = {
    ...
    counters: {
      checkpoint_24h: countSince(24),
      checkpoint_7d: countSince(24 * 7),
      checkpoint_30d: countSince(24 * 30)
    },
    latest: latest
      ? {
          hash: latest.hash,
  ...
  const outDir = path.resolve(root, ".checkpoints", "webhook");
  mkdirSync(outDir, { recursive: true });
  const outPath = path.join(outDir, "checkpoint-webhook-payload.json");
  writeFileSync(outPath, `${JSON.stringify(payload, null, 2)}\n`, "utf8");

  process.stdout.write(`Webhook payload written: ${outPath}\n`);
