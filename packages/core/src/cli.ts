...
export function buildBatchEnvelope(entries: CliBatchEntry[]): CliBatchEnvelope {
...
export function formatBatchJson(entries: CliBatchEntry[]): string {
  return JSON.stringify(buildBatchEnvelope(entries), null, 2);
}
...
function compileGlobToRegExp(pattern: string): RegExp {
  ...
  for (let i = 0; i < pattern.length; i += 1) {
    ...
    if (ch === "*") {
      if (pattern[i + 1] === "*") {
        // "**/" matches zero or more directory segments; bare "**" matches anything (including '/')
        if (pattern[i + 2] === "/") {
          body += "(?:.*/)?";
          i += 2;
        } else {
...
function matchesAnyGlob(relativePath: string, patterns: ReadonlyArray<string>): boolean {
...
async function listBatchInputFiles(
  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (hasExclude && matchesAnyGlob(rel, options.exclude!)) continue;
      if (hasInclude) {
        if (!matchesAnyGlob(rel, options.include!)) continue;
      } else if (!BATCH_INPUT_EXTENSIONS.test(entry.name)) {
        continue;
      }
...
export async function main(argv: readonly string[], io: CliIo = {}): Promise<number> {
  ...
  if (parsed.inputDir !== undefined) {
    let files: string[];
    try {
      files = await listBatchInputFiles(parsed.inputDir, {
    ...
    let output: string;
    if (parsed.format === "json") {
      output = formatBatchJson(entries);
