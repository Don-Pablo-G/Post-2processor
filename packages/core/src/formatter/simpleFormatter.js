export function simpleFormat(ast, style) {
    return ast.blocks
        .filter((block) => {
        // Drop structural lines like "%" that produce no parseable words/comments.
        if (block.words.length === 0 && !block.comment)
            return false;
        return true;
    })
        .filter((block) => {
        if (!style.removeStandaloneOptionalStops)
            return true;
        if (block.comment)
            return true;
