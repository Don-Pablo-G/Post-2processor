...
import type { ProgramAst, ToolingReport, ToolingReportOptions, ToolUsage, Word } from "../types.js";

type Orientation = { a: number; b: number; c: number };
...
export function buildToolingReport(
  ...
  const tools = new Map<number, ToolUsage>();
  const warnings: string[] = [...simulation.warnings];
  warnings.push(...collectDStyleWarnings(ast, options));
  let activeTool: number | null = null;
  let currentOrientation: Orientation = { a: 0, b: 0, c: 0 };
  let currentWorkOffset = "UNKNOWN";
  const workOffsetsSeen = new Set<string>();
  const toolCommentCandidates = collectToolCommentCandidates(ast);
  const pendingToolComp = new Map<
    number,
    { hOffset?: number; hOffsetParameter?: string; dOffset?: number; dOffsetParameter?: string }
  >();
  ...
  for (const entry of simulation.trace) {
    ...
    if (toolInBlock !== null) activeTool = Math.trunc(toolInBlock);
    ...
    if (activeTool !== null && (hOffset !== null || dOffset !== null || hOffsetParameter || dOffsetParameter)) {
      ...
      if (hOffset !== null) comp.hOffset = Math.trunc(hOffset);
      if (dOffset !== null) comp.dOffset = Math.trunc(dOffset);
      if (hOffsetParameter) comp.hOffsetParameter = hOffsetParameter;
      if (dOffsetParameter) comp.dOffsetParameter = dOffsetParameter;
      ...
      if (activeTool !== null) {
        ...
        if (!existing) {
          ...
          tools.set(activeTool, {
            ...
            selectedToolComment: pickSelectedToolComment(
              options?.toolCommentSelections?.[activeTool],
              toolCommentCandidates.get(activeTool) ?? [],
              autoSelectToolComments
            ),
          ...
          if (hOffset !== null) existing.hOffset = Math.trunc(hOffset);
          if (dOffset !== null) existing.dOffset = Math.trunc(dOffset);
          if (hOffsetParameter) existing.hOffsetParameter = existing.hOffsetParameter ?? hOffsetParameter;
          if (dOffsetParameter) existing.dOffsetParameter = existing.dOffsetParameter ?? dOffsetParameter;
          if (!existing.workOffsetsUsed.includes(currentWorkOffset)) existing.workOffsetsUsed.push(currentWorkOffset);
        } else {
          if (hOffset !== null) existing.hOffset = existing.hOffset ?? Math.trunc(hOffset);
          if (dOffset !== null) existing.dOffset = existing.dOffset ?? Math.trunc(dOffset);
          if (hOffsetParameter) existing.hOffsetParameter = existing.hOffsetParameter ?? hOffsetParameter;
          if (dOffsetParameter) existing.dOffsetParameter = existing.dOffsetParameter ?? dOffsetParameter;
          if (!existing.workOffsetsUsed.includes(currentWorkOffset)) existing.workOffsetsUsed.push(currentWorkOffset);
        }
  ...
  return {
    ...
    setupInstructions,
    warnings,
    printable80mm: render80mmReport(
      list,
      programLowestZ,
      Array.from(workOffsetsSeen).sort(),
      setupInstructions,
      warnings,
      options
    )
...
function collectDStyleWarnings(ast: ProgramAst, options?: ToolingReportOptions): string[] {
  const issues: string[] = [];
...
function parseWords(code: string): Word[] {
  return Array.from(
    code.matchAll(/([A-Z])\s*([+\-]?(?:\d+(?:\.\d*)?|\.\d+|\#\d+|\[[^\]]+\]))/g)
  ).map((m) => ({ letter: m[1], value: m[2] }));
}

function evalNumeric(token: string, vars: Record<string, number>): number {
  if (/^#\d+$/.test(token)) return vars[token] ?? 0;
  if (/^[+\-]?(?:\d+(?:\.\d*)?|\.\d+)$/.test(token)) return Number(token);
  return Number.NaN;
}

function getLastValue(words: Word[], letter: string, vars: Record<string, number>): number | null {
...
function getLastParameterRef(words: Word[], letter: string): string | undefined {
  const match = words.filter((w) => w.letter === letter).at(-1);
...
function detectWorkOffset(words: Word[], vars: Record<string, number>): string | null {
  const gValues = words
    .filter((w) => w.letter === "G")
    .map((w) => Math.trunc(evalNumeric(w.value, vars)))
    .filter((v) => Number.isFinite(v));
  const standard = gValues.find((g) => g >= 54 && g <= 59);
  if (standard !== undefined) return `G${standard}`;

  const g154 = gValues.find((g) => g === 154);
  if (g154 !== undefined) {
    const p = getLastValue(words, "P", vars);
    if (p !== null) return `G154 P${Math.trunc(p)}`;
    return "G154";
  }
  return null;
}

function updateOrientation(base: Orientation, words: Word[], vars: Record<string, number>): Orientation {
...
function combinedTiltRadians(orientation?: Orientation): number {
...
function render80mmReport(
  ...
  if (options?.fiveAxis?.enabled) lines.push(pad(`MODE: ${options.fiveAxis.machine.toUpperCase()}`, width));
  lines.push("-".repeat(width));
...
function collectToolCommentCandidates(ast: ProgramAst): Map<number, string[]> {
  const byTool = new Map<number, Array<{ comment: string; score: number; order: number }>>();
  let orderCounter = 0;
  ast.blocks.forEach((block, index) => {
    const code = cleanCode(block.raw).toUpperCase();
  ...
  for (const [tool, list] of byTool.entries()) {
    const dedup = new Map<string, { score: number; order: number }>();
...
function extractComments(raw: string): string[] {
  const comments: string[] = [];
  for (const m of raw.matchAll(/\(([^)]{1,120})\)/g)) {
    comments.push(m[1].trim());
  }
  const semicolon = raw.match(/;(.+)$/);
  if (semicolon) comments.push(semicolon[1].trim());
  return comments;
}

function looksLikeToolName(comment: string): boolean {
  const upper = comment.toUpperCase();
  if (upper.includes("OPTIONAL STOP")) return false;
  if (/^M0?1$/.test(upper)) return false;
  if (/^(CHECK|VERIFY|NOTE)\b/.test(upper)) return false;
  return comment.length >= 2;
}

function pickSelectedToolComment(
  explicitSelection: string | undefined,
  candidates: string[],
  autoSelectEnabled: boolean
): string | undefined {
  if (explicitSelection && explicitSelection.trim().length > 0) return explicitSelection;
  if (!autoSelectEnabled) return undefined;
  return candidates[0];
}
...
function buildSetupInstructions(
  ...
  if (options?.includeSetupInstructions === false) return [];
  const lines: string[] = [];
