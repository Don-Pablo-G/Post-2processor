...
export function buildProveoutProgram(ast, initialState) {
    const advisor = analyzeProgram(ast, initialState);
    const stopBlocks = new Map(advisor.optionalStopSuggestions.map((s) => [s.blockIndex, s.suggestedLine]));
    let inserted = 0;
    const lines = [];
...
export function applyProveoutMarkers(code, markerLines) {
...
export function removeProveoutMarkers(code) {
    const lines = code.split(/\r?\n/);
    let markersRemoved = 0;
    const output = lines.filter((line) => {
        const isMarker = line.includes("PROVEOUT MODE ENABLED") ||
...
//# sourceMappingURL=proveout.js.map