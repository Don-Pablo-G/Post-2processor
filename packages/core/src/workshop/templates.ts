...
const DEFAULT_SETTINGS: TemplateLibrary["settings"] = {
  parameterDefaults: {
    "haas-ngc": {
...
export function exportTemplateLibrary(templates: ProgramTemplate[] = TEMPLATES): TemplateLibrary {
  return {
    templates,
...
export function importTemplateLibrary(sourceJson: string): TemplateLibrary {
  try {
    const parsed = JSON.parse(sourceJson) as {
      templates?: ProgramTemplate[];
      settings?: TemplateLibrary["settings"];
    };
...
function isValidTemplate(value: ProgramTemplate): boolean {
  return Boolean(value?.id && value?.name && value?.description && value?.code);
}

function sanitizeSettings(settings: TemplateLibrary["settings"] | undefined): TemplateLibrary["settings"] {
  const defaults = settings?.parameterDefaults ?? DEFAULT_SETTINGS?.parameterDefaults ?? {};
  const sanitized: NonNullable<TemplateLibrary["settings"]>["parameterDefaults"] = {};
