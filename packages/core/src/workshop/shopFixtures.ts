...
export async function importShopFixture(input: ImportShopFixtureInput): Promise<ImportShopFixtureResult> {
  ...
  const relativeFixturePath = toPosixPath(
    path.relative(input.fixturesRootDirectory, fixtureFilePath).replace(/\\/g, "/")
  );

  const manifestRaw = await fs.readFile(manifestPath, "utf8");
  const manifest = JSON.parse(manifestRaw) as ShopManifest;
...
export async function validateShopFixturesManifest(
  ...
  for (let i = 0; i < manifest.fixtures.length; i += 1) {
    ...
    assertOptionalStringArray(
      fixture.expectations.expectedFindingCodes,
      `${prefix}.expectations.expectedFindingCodes`
    );
  }
...
export async function analyzeShopFixtures(
  ...
  const byController: AnalyzeShopFixturesResult["byController"] = {
    "haas-ngc": 0,
    "haas-legacy": 0,
    fanuc: 0
  };

  const items: FixtureHealthItem[] = [];
  for (const fixture of manifest.fixtures) {
    byController[fixture.controller] += 1;
    const strictMode = Boolean(
      fixture.expectations.expectedFindingCodes && fixture.expectations.expectedFindingCodes.length > 0
    );
...
export async function previewShopFixtureAutoFixes(
  ...
  const changes: ShopFixtureAutoFixChange[] = [];
...
export async function applyShopFixtureAutoFixes(
  ...
  const filteredChanges = preview.changes.filter((change) => {
    if (change.kind !== "controller_mismatch") return true;
    return confidenceRank(change.confidence ?? "low") >= confidenceRank(minConfidence);
  });
  const changesToApply = new Set(
    filteredChanges.map((c) => `${c.fixtureId}:${c.kind}:${c.field}:${c.from}:${c.to}`)
  );

  const parsed = JSON.parse(preview.updatedManifestJson) as ShopManifest;
  for (const fixture of parsed.fixtures) {
    ...
    for (const change of fixtureChanges) {
      ...
      if (!changesToApply.has(key)) {
        fixture.controller = change.from as typeof fixture.controller;
      }
    }
  }
...
function detectControllerProfile(code: string): "haas-ngc" | "haas-legacy" | "fanuc" {
...
function detectControllerProfileWithConfidence(code: string): {
  controller: "haas-ngc" | "haas-legacy" | "fanuc";
  confidence: "high" | "medium" | "low";
} {
  const header = code.toUpperCase().split(/\r?\n/).slice(0, 40).join("\n");
  if (/\bFANUC\b/.test(header)) return { controller: "fanuc", confidence: "high" };
  if (/\bNGC\b/.test(header) || /\bG234\b/.test(header) || /\bDWO\b/.test(header)) {
    return { controller: "haas-ngc", confidence: "high" };
  }
  if (/\bHAAS\b/.test(header)) return { controller: "haas-legacy", confidence: "medium" };
...
function buildAutoFixFingerprint(input: PreviewShopFixtureAutoFixesInput, changes: ShopFixtureAutoFixChange[]): string {
...
function buildBackupPath(manifestPath: string, changesCount: number): string {
