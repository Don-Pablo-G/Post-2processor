...
function collectSafetyFindings(ast, options) {
    const findings = [];
    let seenG43 = false;
    let seenSafeStart = false;
    let seenM30 = false;
    let currentTool = null;
    const toolHasLengthComp = new Map();
    const usedToolOffsets = new Map();
    ast.blocks.forEach((block, index) => {
        ...
        if (t)
            currentTool = parseIntSafe(t.value);
...
function buildCriticalEvents(ast, initialState) {
    ...
    sim.trace.forEach((entry) => {
        ...
        if (z !== null) {
            if (deepest === null || z < deepest.z)
                deepest = { z, block: entry.blockIndex };
    ...
    if (deepest) {
        events.push({ kind: "deepest_z", blockIndex: deepest.block, description: `Deepest Z reached: ${deepest.z}.` });
    }
...
function suggestSetupOptimizations(ast) {
    const toolsInOrder = [];
    ast.blocks.forEach((block) => {
        const code = cleanCode(block.raw).toUpperCase();
        const tMatch = code.match(/\bT\s*(\d+)/);
        if (tMatch)
            toolsInOrder.push(Number(tMatch[1]));
    });
    if (toolsInOrder.length < 3)
        return [];
    let changes = 0;
    for (let i = 1; i < toolsInOrder.length; i += 1) {
        if (toolsInOrder[i] !== toolsInOrder[i - 1])
            changes += 1;
    }
...
function buildOperatorView(ast) {
    return ast.blocks
        .map((block) => {
        const raw = block.raw.trim();
        const code = cleanCode(raw).toUpperCase();
        if (/\bM\s*6\b/.test(code))
            return `(OPERATOR: TOOL CHANGE CHECK)\n${raw}`;
        if (/\bG\s*43\b/.test(code))
            return `(OPERATOR: LENGTH/OFFSET ACTIVE)\n${raw}`;
        if (/\bG\s*(5[4-9]|154)\b/.test(code))
            return `(OPERATOR: VERIFY WORK OFFSET)\n${raw}`;
        if (/\bG\s*(81|82|83|84|85|86|87|88|89)\b/.test(code))
            return `(OPERATOR: CANNED CYCLE START)\n${raw}`;
        return raw;
    })
        .join("\n");
}
function collectEnvelopeFindings(ast, options) {
    const findings = [];
    if (!options?.stock && (!options?.clampZones || options.clampZones.length === 0))
        return findings;
    ast.blocks.forEach((block, index) => {
        const code = cleanCode(block.raw).toUpperCase();
...
function collectFusionStyleFindings(ast) {
    const findings = [];
    let hasG94 = false;
    let hasG95 = false;
    let hasMetric = false;
    ...
    ast.blocks.forEach((block, index) => {
        const code = cleanCode(block.raw).toUpperCase();
        if (/\bG\s*94\b/.test(code))
            hasG94 = true;
        if (/\bG\s*95\b/.test(code))
            hasG95 = true;
        if (/\bG\s*21\b/.test(code))
            hasMetric = true;
        if (/\bG\s*20\b/.test(code))
            hasImperial = true;
...
function buildOptionalStopSuggestions(ast, events) {
    const suggestions = [];
...
function buildParameterFrontMatter(ast) {
    const used = new Set();
    ast.blocks.forEach((b) => {
        for (const m of b.raw.matchAll(/#(\d+)/g))
            used.add(Number(m[1]));
    });
    ...
    if (assigned.length < 4)
        return "(INFO: NOT ENOUGH FREE LOCAL MACROS FOR FRONT-MATTER)";
    return [
...
function computeReadyToRunScore(findings) {
...
function extractAxis(code, axis) {
