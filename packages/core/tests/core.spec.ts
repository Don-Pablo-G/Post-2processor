...
describe("Haas NGC profile package (@cnc/profile-haas-ngc)", () => {
  ...
  it("warns spindle speed while coolant is on", () => {
    const ast = parse("O1\nT1 M6\nG54\nS1200 M3\nM8\nS800\nM9\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("Spindle speed (S) while coolant is still on")
      )
    ).toBe(true);
  });

  it("warns program ends with G52 local offset still applied", () => {
    const ast = parse("O1\nT1 M6\nG54\nS1200 M3\nG52 X10.\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("Program ends with G52 local offset still applied")
      )
    ).toBe(true);
  });

  it("warns H offset while canned cycle is active", () => {
    const ast = parse(
      "O1\nT1 M6\nG54\nS1200 M3\nG81 Z-1. R0.1 F10.\nH2\nG80\nM5\nM30",
      haasNgcProfilePackaged
    );
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("H offset word while a canned cycle is still active")
      )
    ).toBe(true);
  });

  it("warns H offset while cutter compensation is active", () => {
    const ast = parse(
      "O1\nT1 M6\nG54\nG43 H1 Z25.\nS1200 M3\nG41 D1\nH2\nG40\nM5\nM30",
      haasNgcProfilePackaged
    );
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("H offset word while cutter compensation (G41/G42) is still active")
      )
    ).toBe(true);
  });

  it("warns H offset while rotation is active", () => {
    const ast = parse(
      "O1\nT1 M6\nG54\nS1200 M3\nG68 X0 Y0 R45.\nH2\nG69\nM5\nM30",
      haasNgcProfilePackaged
    );
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("H offset word while coordinate rotation (G68) is still active")
      )
    ).toBe(true);
  });

  it("warns H offset while scaling is active", () => {
    const ast = parse("O1\nT1 M6\nG54\nS1200 M3\nG51 P2.\nH2\nG50\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("H offset word while scaling (G51) is still active")
      )
    ).toBe(true);
  });

  it("warns D offset while canned cycle is active", () => {
    const ast = parse(
      "O1\nT1 M6\nG54\nS1200 M3\nG81 Z-1. R0.1 F10.\nD2\nG80\nM5\nM30",
      haasNgcProfilePackaged
    );
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("D offset word while a canned cycle is still active")
      )
    ).toBe(true);
  });

  it("warns D offset while rotation is active", () => {
    const ast = parse(
      "O1\nT1 M6\nG54\nS1200 M3\nG68 X0 Y0 R45.\nD2\nG69\nM5\nM30",
      haasNgcProfilePackaged
    );
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("D offset word while coordinate rotation (G68) is still active")
      )
    ).toBe(true);
  });

  it("warns D offset while scaling is active", () => {
    const ast = parse("O1\nT1 M6\nG54\nS1200 M3\nG51 P2.\nD2\nG50\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("D offset word while scaling (G51) is still active")
      )
    ).toBe(true);
  });

  it("warns D offset while incremental is active", () => {
    const ast = parse("O1\nT1 M6\nG54\nG91\nS1200 M3\nD2\nG90\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("D offset word while incremental mode (G91) is active")
      )
    ).toBe(true);
  });

  it("warns H offset while incremental is active", () => {
    const ast = parse("O1\nT1 M6\nG54\nG91\nS1200 M3\nH2\nG90\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("H offset word while incremental mode (G91) is active")
      )
    ).toBe(true);
  });

  it("warns H offset while coolant is on", () => {
    const ast = parse("O1\nT1 M6\nG54\nS1200 M3\nM8\nH2\nM9\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("H offset word while coolant is still on")
      )
    ).toBe(true);
  });

  it("warns D offset while cutter compensation is active", () => {
    const ast = parse(
      "O1\nT1 M6\nG54\nG43 H1 Z25.\nS1200 M3\nG41 D1\nD2\nG40\nM5\nM30",
      haasNgcProfilePackaged
    );
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("D offset word while cutter compensation (G41/G42) is still active")
      )
    ).toBe(true);
  });

  it("warns D offset while coolant is on", () => {
    const ast = parse("O1\nT1 M6\nG54\nS1200 M3\nM8\nD2\nM9\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("D offset word while coolant is still on")
      )
    ).toBe(true);
  });

  it("warns Q word while cutter compensation is active", () => {
    const ast = parse(
      "O1\nT1 M6\nG54\nG43 H1 Z25.\nS1200 M3\nG41 D1\nQ0.1\nG40\nM5\nM30",
      haasNgcProfilePackaged
    );
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("Q word while cutter compensation (G41/G42) is still active")
      )
    ).toBe(true);
  });

  it("warns Q word while canned cycle is active", () => {
    const ast = parse(
      "O1\nT1 M6\nG54\nS1200 M3\nG81 Z-1. R0.1 F10.\nQ0.1\nG80\nM5\nM30",
      haasNgcProfilePackaged
    );
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("Q word while a canned cycle is still active")
      )
    ).toBe(true);
  });

  it("warns Q word while rotation is active", () => {
    const ast = parse(
      "O1\nT1 M6\nG54\nS1200 M3\nG68 X0 Y0 R45.\nQ0.1\nG69\nM5\nM30",
      haasNgcProfilePackaged
    );
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("Q word while coordinate rotation (G68) is still active")
      )
    ).toBe(true);
  });

  it("warns Q word while scaling is active", () => {
    const ast = parse("O1\nT1 M6\nG54\nS1200 M3\nG51 P2.\nQ0.1\nG50\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("Q word while scaling (G51) is still active")
      )
    ).toBe(true);
  });

  it("warns Q word while incremental is active", () => {
    const ast = parse("O1\nT1 M6\nG54\nG91\nS1200 M3\nQ0.1\nG90\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("Q word while incremental mode (G91) is active")
      )
    ).toBe(true);
  });

  it("warns Q word while coolant is on", () => {
    const ast = parse("O1\nT1 M6\nG54\nS1200 M3\nM8\nQ0.1\nM9\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) => i.message.includes("Q word while coolant is still on"))
    ).toBe(true);
  });
  ...
  it("warns R word while cutter compensation is active", () => {
    ...
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("R word while cutter compensation (G41/G42) is still active")
      )
    ).toBe(true);
  });

  it("warns R word while rotation is active", () => {
    const ast = parse(
      "O1\nT1 M6\nG54\nS1200 M3\nG68 X0 Y0 R45.\nR0.1\nG69\nM5\nM30",
      haasNgcProfilePackaged
    );
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("R word while coordinate rotation (G68) is still active")
      )
    ).toBe(true);
  });

  it("warns R word while scaling is active", () => {
    const ast = parse("O1\nT1 M6\nG54\nS1200 M3\nG51 P2.\nR0.1\nG50\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("R word while scaling (G51) is still active")
      )
    ).toBe(true);
  });

  it("warns R word while incremental is active", () => {
    const ast = parse("O1\nT1 M6\nG54\nG91\nS1200 M3\nR0.1\nG90\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("R word while incremental mode (G91) is active")
      )
    ).toBe(true);
  });

  it("warns R word while coolant is on", () => {
    const ast = parse("O1\nT1 M6\nG54\nS1200 M3\nM8\nR0.1\nM9\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) => i.message.includes("R word while coolant is still on"))
    ).toBe(true);
  });

  it("warns R word while tool length is active", () => {
    const ast = parse(
      "O1\nT1 M6\nG54\nG43 H1 Z25.\nS1200 M3\nR0.1\nG49\nM5\nM30",
      haasNgcProfilePackaged
    );
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("R word while tool length compensation (G43) is still active")
      )
    ).toBe(true);
  });

  it("warns P word while cutter compensation is active", () => {
    const ast = parse(
      "O1\nT1 M6\nG54\nG43 H1 Z25.\nS1200 M3\nG41 D1\nP100\nG40\nM5\nM30",
      haasNgcProfilePackaged
    );
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("P word while cutter compensation (G41/G42) is still active")
      )
    ).toBe(true);
  });

  it("warns P word while rotation is active", () => {
    const ast = parse(
      "O1\nT1 M6\nG54\nS1200 M3\nG68 X0 Y0 R45.\nP100\nG69\nM5\nM30",
      haasNgcProfilePackaged
    );
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("P word while coordinate rotation (G68) is still active")
      )
    ).toBe(true);
  });

  it("warns P word while scaling is active", () => {
    const ast = parse("O1\nT1 M6\nG54\nS1200 M3\nG51 P2.\nP100\nG50\nM5\nM30", haasNgcProfilePackaged);
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("P word while scaling (G51) is still active")
      )
    ).toBe(true);
  });

  it("warns P word while tool length is active", () => {
    const ast = parse(
      "O1\nT1 M6\nG54\nG43 H1 Z25.\nS1200 M3\nP100\nG49\nM5\nM30",
      haasNgcProfilePackaged
    );
    expect(
      lint(ast, haasNgcProfilePackaged).some((i) =>
        i.message.includes("P word while tool length compensation (G43) is still active")
      )
    ).toBe(true);
  });
